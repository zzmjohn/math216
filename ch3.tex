...
\begin{exercise} % TODO expand and draw
  The image presheaf is defined by~$\image^{\pre}\phi(U)=\phi(U)(\mathcal{F}(U))$. 
\end{exercise}
...
\begin{exercise}
  \begin{enumerate}
    \item If~$y\in Z$, then~$i^{-1}(U)=Z\cap U$ where~$y\in U$ open. Now~$(i_*\mathcal{F})_y$ is defined as the direct system of the~$U\cap Z$ because~$U\cap Z$ is open in the closed subspace~$Z$. Therefore~$(i_*\mathcal{F})_y=\mathcal{F}_y$ in this case. For~$y\notin Z$ we have by definition the one element-set as the stalk.
