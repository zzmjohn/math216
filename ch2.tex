...
\begin{exercise}
  \begin{enumerate}
    \item Let's tediously check all module axioms. Take~$b_1,b_2\in B$ and~$b\otimes m,b'\otimes m'\in B\otimes_A M$. We have
      \begin{description}
        \item[$r(x+y)=rx+ry$] Using the axioms of the tensor product we obtain
          \begin{equation}
            \begin{aligned}
              b_1(b\otimes m+b'\otimes m') &= b_1\left( (b+b')\otimes (m+m') \right) \\
              &= \left( b_1(b+b') \right)\otimes(m+m') \\
              &= (b_1b+b_1b')\otimes (m+m') \\
              &= b_1b\otimes (m+m')+b_1(b'\otimes(m+m') \\
              &= b\otimes b_1(m+m')+b'\otimes b_1(m+m') \\
              &=
            \end{aligned}
          \end{equation}
